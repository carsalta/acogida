...
import QuestionsEditor from '../components/QuestionsEditor'; // <- ojo a la ruta
...
export default function Admin() {
    ...
    // Exporta overrides actuales para compartir (subiéndolos a public/overrides.json)
    const downloadOverrides = () => {
        // Exporta todo cfg como overrides compartidos
        const shared = { ...(cfg || {}) };
    ...
    const setBrandField = (key, val) => {
        ...
        if (site && next.sites?.[site]) {
            next.sites = {
                ...
                [site]: {
                    ...(next.sites[site] || {}),
                    brand: { ...(next.sites[site]?.brand || {}), [key]: val }
                }
            };
        } else {
    ...
    return (
        <div style={{ padding: 24, display: 'grid', gap: 24 }}>
                ...
                <div style={{ display: 'grid', gap: 12, gridTemplateColumns: '1fr 1fr' }}>
                    ...
                    <label style={{ display: 'grid', gap: 4, gridColumn: '1 / span 2' }}>
                        ...
                        <p style={{ fontSize: 12, color: '#64748b' }}>
                            Sube el archivo a <code>public/brand/</code> y referencia con <code>brand/tu-logo.png</code> (sin barra inicial),
                            o usa un enlace público.
                        </p>
                    </label>
                    {activeBrand?.logo && (
            ...
            <Section
                ...
                right={
                    <div style={{ display: 'flex', gap: 8 }}>
                        <button className="btn btn-outline" onClick={downloadCfg}>Descargar config.json</button>
                        <button className="btn btn-outline" onClick={downloadOverrides}>Descargar overrides.json</button>
                    </div>
                }
            >
