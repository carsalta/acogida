...
export default function Admin(){
  ...
  const currentVideos = useMemo(()=>{
    if(!cfg) return {}; return site ? (cfg.sites?.[site]?.videos || {}) : (cfg.videos || {});
  }, [cfg, site]);
  ...
  const setVideoPath = (type, lang, path) => {
    ...
    if (site && next.sites?.[site]){
      next.sites = { ...next.sites, [site]: { ...(next.sites[site]||{}), videos: { ...(next.sites[site]?.videos||{}), [type]: { ...(next.sites[site]?.videos?.[type]||{}), [lang]: path } } } };
    } else {
      next.videos = { ...(next.videos||{}), [type]: { ...(next.videos?.[type]||{}), [lang]: path } };
    }
    saveOverrides(next);
  };

  const makeQr = async (key, url) => { const data = await QRCode.toDataURL(url, { margin: 1, scale: 6 }); setQrImg(m => ({ ...m, [key]: data })); };
  const downloadCfg = () => { const blob = new Blob([JSON.stringify(cfg || {}, null, 2)], { type: 'application/json' }); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='config.json'; a.click(); URL.revokeObjectURL(a.href); };

  const activeBrand = site ? (cfg?.sites?.[site]?.brand || {}) : (cfg?.brand || {});
  const setBrandField = (key, val) => {
    const next = { ...(cfg || {}) };
    if (site && next.sites?.[site]){
      next.sites = { ...next.sites, [site]: { ...(next.sites[site]||{}), brand: { ...(next.sites[site]?.brand||{}), [key]: val } } };
    } else {
  ...
  const mailEnabled = !!cfg?.mail?.enabled; const mailApiBase = cfg?.mail?.apiBase || ''; const mailApiKey = cfg?.mail?.apiKey || ''; const mailCC = (cfg?.mail?.cc || []).join(', ');
  ...
  const onChangeDefaultLang = (lang) => { const safe = getEnabledLangs(cfg); if (!safe.includes(lang)) return; saveOverrides({ ...(cfg || {}), defaultLang: lang }); };
  ...
  return (
    <div style={{padding:24, display:'grid', gap:24}}>
      ...
      <Section id="flags" title="Ajustes / Feature flags" defaultOpen={true}>
        <label style={{display:'flex',alignItems:'center',gap:8}}><input type="checkbox" checked={!!cfg.allowSeek} onChange={()=>toggle('allowSeek')} />Permitir adelantar (seek)</label>
        <label style={{display:'flex',alignItems:'center',gap:8,marginTop:8}}><input type="checkbox" checked={!!cfg.allowSubtitles} onChange={()=>toggle('allowSubtitles')} />Habilitar subtítulos</label>
      </Section>
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:12}}>
              {ALL_LANGS.map(l => (
                <label key={l} style={{display:'flex',alignItems:'center',gap:8}}>
                  <input type="checkbox" checked={enabledLangs.includes(l)} onChange={e=>onToggleLang(l, e.target.checked)} />
                  {LANG_LABEL[l]}
                </label>
              ))}
            </div>
        ...
        <div style={{display:'grid',gap:12, gridTemplateColumns:'1fr 1fr'}}>
          ...
          {activeBrand?.logo && (
            <div style={{gridColumn:'1 / span 2'}}>
              ...
              <img src={activeBrand.logo.startsWith('http')? activeBrand.logo : (import.meta.env.BASE_URL + activeBrand.logo.replace(/^\//,''))} alt="preview-logo" style={{height:48,width:'auto',border:'1px solid #e2e8f0',borderRadius:6,objectFit:'contain'}} />
            </div>
          )}
        </div>
      </Section>

      <Section id="videos" title={`Vídeos por idioma (${site || 'global'})`} defaultOpen={true} right={<button className="btn btn-outline" onClick={downloadCfg}>Descargar config.json</button>}>
            ...
            {enabledLangs.map(lang => {
              ...
              return (
                <div key={lang} style={{display:'grid',gap:8, gridTemplateColumns:'80px 1fr', alignItems:'center', marginTop:8}}>
                  ...
                  <input className="border rounded" style={{padding:'4px 8px'}} placeholder={`videos/${lang}/${type}.mp4`} defaultValue={current} onBlur={(e)=> setVideoPath(type, lang, e.target.value.trim())} />
                </div>
              );
            })}
          </div>
      ...
      <Section id="mail" title="Correo (M365 Graph vía API)" defaultOpen={false}>
        <div style={{display:'grid',gap:12, gridTemplateColumns:'1fr 1fr'}}>
          <label style={{display:'grid',gap:4}}><span style={{fontSize:14,color:'#334155'}}>Habilitado</span>
            <input type="checkbox" defaultChecked={mailEnabled} onChange={(e)=> saveOverrides({ ...(cfg || {}), mail: { ...(cfg?.mail||{}), enabled: e.target.checked } })} />
          </label>
          <label style={{display:'grid',gap:4}}><span style={{fontSize:14,color:'#334155'}}>API Base (Azure Function)</span>
            <input className="border rounded" style={{padding:'4px 8px'}} placeholder="https://<tu-funcion>.azurewebsites.net/api" defaultValue={mailApiBase} onBlur={(e)=> saveOverrides({ ...(cfg || {}), mail: { ...(cfg?.mail||{}), apiBase: e.target.value.trim() } })} />
          </label>
          <label style={{display:'grid',gap:4}}><span style={{fontSize:14,color:'#334155'}}>API Key (X-API-KEY)</span>
            <input className="border rounded" style={{padding:'4px 8px'}} placeholder="clave secreta" defaultValue={mailApiKey} onBlur={(e)=> saveOverrides({ ...(cfg || {}), mail: { ...(cfg?.mail||{}), apiKey: e.target.value.trim() } })} />
          </label>
          <label style={{display:'grid',gap:4}}><span style={{fontSize:14,color:'#334155'}}>CC (separar por coma)</span>
            <input className="border rounded" style={{padding:'4px 8px'}} placeholder="rrhh@empresa.com, prevencion@empresa.com" defaultValue={mailCC} onBlur={(e)=> saveOverrides({ ...(cfg || {}), mail: { ...(cfg?.mail||{}), cc: e.target.value.split(',').map(s=>s.trim()).filter(Boolean) } })} />
          </label>
        </div>
      </Section>

      <Section id="share" title="Compartir (enlaces + QR)" defaultOpen={false}>
        <p style={{fontSize:14,color:'#64748b',marginBottom:8}}>Genera enlaces por tipo e idioma habilitado. Si hay site, añade <code>?site=...</code>.</p>
              ...
              return (
                <div style={{padding:12,border:'1px solid #e2e8f0',borderRadius:8}} key={k}>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                    ...
                    <button className="btn" onClick={async()=>{ const data=await QRCode.toDataURL(url,{margin:1,scale:6}); setQrImg(m=>({...m,[k]:data})); }}>Generar QR</button>
                  </div>
