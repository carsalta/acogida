...
function abs(p) { if (!p) return ''; if (p.startsWith('http') || p.startsWith('data:')) return p; return p.startsWith('/') ? (import.meta.env.BASE_URL + p.replace(/^\/,/, '').replace(/^\/+/, '')) : (import.meta.env.BASE_URL + p); }
    ...
    const onQuizPassed = async () => {
        ...
        try {
            if (cfg?.mail?.enabled) {
                ...
                const subj = (lang === 'es' ? 'Certificado de Inducción' : (lang === 'pt' ? 'Certificado de Instruções de Segurança' : (lang === 'fr' ? 'Certificat de Sécurité' : (lang === 'de' ? 'Sicherheitszertifikat' : 'Induction Certificate'))));
