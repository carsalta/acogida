...
function tracks(type, lang, langs){ if(!type) return []; return (langs||[]).map(l=>({ src:`${import.meta.env.BASE_URL}subtitles/${type}.${l}.vtt`, srclang:l, label: LANG_LABEL[l], default: lang===l })); }

function abs(p){ if(!p) return ''; if(p.startsWith('http')||p.startsWith('data:')) return p; return p.startsWith('/')? (import.meta.env.BASE_URL + p.replace(/^\//,'')) : (import.meta.env.BASE_URL + p); }
...
export default function App(){
  ...
  useEffect(()=>{ const p = new URLSearchParams(location.search); if (p.get('lang')) setLang(p.get('lang')); if(p.get('id')) setRoute('verify'); if(p.get('site')) setSite(p.get('site')); if(p.get('kiosk')==='1'){ setKiosk(true); document.body.classList.add('kiosk'); if (document.fullscreenEnabled) document.documentElement.requestFullscreen().catch(()=>{}); } },[]);
  useEffect(()=>{ if(cfg?.forceLang) setLang(cfg.forceLang); }, [cfg?.forceLang]);
  useEffect(()=>{ if(!site){ const first = Object.keys(cfg?.sites||{})[0] || ''; setSite(first); } }, [cfg, site]);
  ...
  const onFormSubmit = (e)=>{ e.preventDefault(); const f=new FormData(e.target); const data={ name:f.get('name')?.trim(), idDoc:f.get('idDoc')?.trim(), company:f.get('company')?.trim(), email:f.get('email')?.trim() }; if(!data.name||!data.idDoc||!data.company||!data.email){ alert(lang==='es'?'Completa todos los campos':'Fill all fields'); return; } setParticipant(data); localStorage.setItem('participant', JSON.stringify(data)); setRoute('video'); };
  const onVideoFinished = ()=> setRoute('quiz');
  const onQuizPassed = async()=>{ const id=uuidv4(); const issue=new Date(); const months= type==='contrata'?12:24; const expiry=new Date(issue); expiry.setMonth(expiry.getMonth()+months); const verifyUrl=`${location.origin}${location.pathname}?id=${id}`; const certs=JSON.parse(localStorage.getItem('certs')||'[]'); certs.push({id, expiry: expiry.toISOString()}); localStorage.setItem('certs', JSON.stringify(certs)); const blob = await buildCertificate({ lang, name: participant.name, idDoc: participant.idDoc, company: participant.company, type, certId: id, issueDate: issue.toLocaleDateString(lang), expiryDate: expiry.toLocaleDateString(lang), verifyUrl });
    try { if (cfg?.mail?.enabled){ const b64 = await blobToBase64(blob); const subj = (lang==='es'? 'Certificado de Inducción' : (lang==='pt'?'Certificado de Instruções de Segurança' : (lang==='fr'?'Certificat de Sécurité' : (lang==='de'?'Sicherheitszertifikat':'Induction Certificate')))); const html = `<p>${lang==='es'?'Adjuntamos su certificado de inducción.':'Please find attached your induction certificate.'}</p>`; await sendMail({ apiBase: cfg?.mail?.apiBase, apiKey: cfg?.mail?.apiKey, to: participant.email, cc: cfg?.mail?.cc||[], subject: subj, html, attachment: { name: `certificado-${id}.pdf`, mime: 'application/pdf', contentBase64: b64 } }); } } catch(e){ console.error('sendMail error', e); }
    const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download=`certificado-${id}.pdf`; a.click(); URL.revokeObjectURL(url); setRoute('done'); };
    ...
    <div style={{minHeight:'100vh',background:'#f1f5f9',padding:24}}>
      <header style={{display:'flex',alignItems:'center',gap:12,flexWrap:'wrap'}}>
        ...
        {kiosk && document.fullscreenElement && (
          <button className="btn btn-outline" style={{marginLeft:'auto'}} onClick={()=>{ document.exitFullscreen?.(); location.replace(location.pathname); }}>Salir kiosco</button>
        )}
      </header>
      ...
      {route==='form' && (
        <form className="card" style={{marginTop:24,display:'grid',gap:12}} onSubmit={onFormSubmit}>
          <div style={{display:'flex',alignItems:'center',gap:8,fontSize:14,color:'#64748b'}}>
            <button type="button" className="btn btn-outline" onClick={()=>{ setRoute('home'); setType(null); }}>← {lang==='es'?'Volver':'Back'}</button>
            <span>|</span>
      ...
      {route==='quiz' && (
        <div className="card" style={{marginTop:24}}>
          ...
          <Quiz items={(c.questions?.[type]||[])} checkLabel={c.checkBtn} wrongLabel={c.wrong} onPass={onQuizPassed} onRetry={()=>{ setRoute('home'); setType(null); }} retryLabel={c.finishAndRetry || (lang==='es' ? 'Finalizar y nuevo intento' : 'Finish & New attempt')} />
        </div>
      )}
      ...
      {route==='done' && (
        <div className="card" style={{marginTop:24}}>
          ...
          <button className="btn" style={{marginTop:12}} onClick={()=>{ setRoute('home'); setType(null); }}>{c.finish || (lang==='es' ? 'Finalizar' : 'Finish')}</button>
        </div>
      )}
    </div>
  );
...
function AdminBtn({onClick}){ return <button className="btn" style={{marginLeft:'auto'}} onClick={onClick}>Admin</button>; }
