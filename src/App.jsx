...
export default function App() {
    ...
    const onQuizPassed = async () => {
        ...
        const blob = await buildCertificate({
            lang, name: participant.name, idDoc: participant.idDoc, company: participant.company,
            type, certId: id, issueDate: issue.toLocaleDateString(lang), expiryDate: expiry.toLocaleDateString(lang), verifyUrl
        });
        try {
            if (cfg?.mail?.enabled) {
                ...
                await sendMail({
                    ...
                    to: participant.email, cc: cfg?.mail?.cc || [],
                    subject: subj, html,
        ...
        const a = document.createElement('a'); a.href = url; a.download = `certificado-${id}.pdf`; a.click();
            ...
            <header style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                ...
                {kiosk && document.fullscreenElement && (
                    <button className="btn btn-outline" style={{ marginLeft: 'auto' }} onClick={() => {
                        document.exitFullscreen?.(); location.replace(location.pathname);
                    }}>Salir kiosco</button>
                )}
            </header>
            ...
            {route === 'video' && type && (
                <div className="card" style={{ marginTop: 24 }}>
                    ...
                    <VideoGate
                        src={videoUrl}
                        tracks={subTracks}
                        allowSeek={allowSeekBool}
                        allowSubtitles={allowSubsBool}
...
function Field({ label, children }) {
    return (<label style={{ display: 'grid', gap: 4 }}><span style={{ fontSize: 14, color: '#334155' }}>{label}</span>{children}</label>);
}
